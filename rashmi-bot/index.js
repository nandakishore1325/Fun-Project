// Rashmi Bot - Self-contained WhatsApp contextual reply bot (Groq)
    ...
    try {
      if (fs.existsSync(this.earlierChatsPath)) {
        ...
        this.earlierChats = content
          ...
          .join('\n');
        console.log('📜 Loaded earlier chat history with Rashmi');
      } else {
...
class WhatsAppBot {
  ...
  setupEventHandlers() {
    ...
    this.client.on('ready', () => {
      this.isReady = true;
      console.log('✅ Rashmi Bot is ready!');
...
`);
  ...
  async stop() {
    ...
    console.log('👋 Rashmi Bot stopped. Goodbye!');
  }
}
...
bot.start().catch(err => {
  console.error('Failed to start Rashmi bot:', err.message);
  process.exit(1);
});
